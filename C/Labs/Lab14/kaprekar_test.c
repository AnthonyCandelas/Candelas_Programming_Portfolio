 ...
 *
 * Description:
 * Ad-hoc test suite for Kaprekar number function verification.
 */
#include<stdlib.h>
#include<stdio.h>
  ...
  for(i=-10; i<=100000; i++) {
    if(contains(kaprekarNumbers, numKaprekarNumbers, i) != isKaprekar(i)) {
      ...
      if(contains(kaprekarNumbers, numKaprekarNumbers, i)) {
        fprintf(stderr, "i = %d: is a known Kaprekar number, but the function returned false\n", i);
      } else {
        fprintf(stderr, "i = %d: is known NOT to be a Kaprekar number, but the function returned true\n", i);
      }
    } else {
      numCorrect++;
    }
